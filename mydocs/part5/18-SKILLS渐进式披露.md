...
```
                 注册管线                         注入管线                    执行管线
  ┌──────────────────────────────┐   ┌────────────────────────┐   ┌──────────────────────┐
  │ loadSkillsDir.ts             │   │ attachments.ts          │   │ SkillTool.ts          │
  │  · 六源并行加载与去重         │   │  · sentSkillNames 差量   │   │  · 验证/权限/执行三层   │
  │  · 条件技能惰性激活           │   │  · 三种附件类型          │   │  · Inline 展开注入     │
  │  · 动态技能目录发现           │   │                         │   │  · Fork 子 Agent 隔离  │
  └──────────┬───────────────────┘   └──────────┬─────────────┘   └──────┬───────────────┘
             │                                  │                        │
             ▼                                  ▼                        ▼
  ┌──────────────────┐                ┌──────────────────┐   ┌──────────────────────┐
  │ commands.ts      │                │ prompt.ts        │   │ processSlashCmd.tsx  │
  │ getCommands()    │                │ formatCommands   │   │ 用户 /cmd 入口       │
  │ 聚合所有来源      │                │ WithinBudget()   │   │ fork + inline 双路径  │
  └──────────────────┘                └──────────────────┘   └──────────────────────┘
```

| 文件 | 职责 |
|---|---|
| [`loadSkillsDir.ts`](https://github.com/binarylei/claudecode/blob/main/src/skills/loadSkillsDir.ts) | 从 `/skills/` 目录加载技能，管理六个来源的注册、去重、条件技能和动态发现 |
| [`bundledSkills.ts`](https://github.com/binarylei/claudecode/blob/main/src/skills/bundledSkills.ts) | 编译时内置技能的注册入口 |
| [`commands.ts`](https://github.com/binarylei/claudecode/blob/main/src/commands.ts) | 聚合所有来源的技能到统一注册表，`getSkillToolCommands()` 筛选模型可见的技能 |
| [`attachments.ts`](https://github.com/binarylei/claudecode/blob/main/src/utils/attachments.ts) | 附件通道管理，`sentSkillNames` 差量注入 + 三种附件类型 |
| [`prompt.ts`](https://github.com/binarylei/claudecode/blob/main/src/tools/SkillTool/prompt.ts) | 预算约束格式化 `formatCommandsWithinBudget()` + Skill Tool 用法提示 |
| [`prompts.ts`](https://github.com/binarylei/claudecode/blob/main/src/constants/prompts.ts) | System Prompt 组装，`session_guidance` 段注入技能元知识 |
| [`SkillTool.ts`](https://github.com/binarylei/claudecode/blob/main/src/tools/SkillTool/SkillTool.ts) | Skill Tool 实现，验证—权限—执行三层调度，Inline/Fork 双模式 |
| [`processSlashCommand.tsx`](https://github.com/binarylei/claudecode/blob/main/src/utils/processUserInput/processSlashCommand.tsx) | 用户 `/cmd` 入口，fork 模式异步执行 + inline 模式内容展开 |
| [`forkedAgent.ts`](https://github.com/binarylei/claudecode/blob/main/src/utils/forkedAgent.ts) | Fork 上下文准备，与主线共享缓存前缀 |
| [`skillChangeDetector.ts`](https://github.com/binarylei/claudecode/blob/main/src/utils/skills/skillChangeDetector.ts) | 文件变更监听，检测到修改后复位缓存触发全量重发 |

**调用链：**

```
会话启动
  └─ loadAllCommands()           注册阶段：六个来源汇集
      ├─ getBundledSkills()       ← 编译时内置
      ├─ getSkillDirCommands()    ← /skills/ 目录扫描
      ├─ getPluginCommands()      ← 插件分发
      └─ getMCPCommands()         ← MCP 服务器
            │
            ▼
  └─ getSkillToolCommands()     筛选：bundled/skills 始终可见，plugin/MCP 需 description
            │
            ▼
  └─ getSkillListingAttachments()  注入阶段：sentSkillNames 差量过滤
      └─ formatCommandsWithinBudget()  预算约束格式化
            │
            ▼
  └─ $system-reminder 注入       模型可见
            │
            ▼ (模型选中技能)
  └─ SkillTool.call()            执行阶段
      ...
      └─ 调度：
          ...
          └─ inline              → processPromptSlashCommand() 展开注入
```
...
**六个来源的并行加载。** [`getSkillDirCommands()`](https://github.com/binarylei/claudecode/blob/main/src/skills/loadSkillsDir.ts#L638-L804) 以 `Promise.all` 同时加载 managed、user、project、additional directories 四路 `/skills/` 目录和一路 legacy `/commands/` 目录，外加 bundled 技能（独立注册通道）。所有来源输出 `SkillWithPath` 结构，包含技能命令对象及其文件路径。
...
并行加载后进入**去重阶段**。[`getFileIdentity()`](https://github.com/binarylei/claudecode/blob/main/src/skills/loadSkillsDir.ts#L118-L124) 用 `realpath` 解析文件真实路径，`Map<string, Source>` 以 first-wins 策略保留首次出现的来源。选择 `realpath` 而非 inode 是因为虚拟文件系统 / 容器环境 / ExFAT 可能返回 inode 0 或精度丢失——这是一个来自线上 issue 的修复。
