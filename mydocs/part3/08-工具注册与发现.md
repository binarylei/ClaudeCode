...
| `src/constants/tools.ts` | 子 Agent 工具限制的静态 Set（`ALL_AGENT_DISALLOWED_TOOLS` 等） |
| `src/tools/AgentTool/agentToolUtils.ts` | `filterToolsForAgent()` 子 Agent 工具裁剪逻辑 |

调用链从单个工具的定义出发，经注册、过滤、合并，到最终按需暴露给模型：

```
buildTool()                     ← 定义单个工具（五份契约的默认值）
     ↓
getAllBaseTools()               ← 全量注册（feature flag 条件展开）
     ↓
filterToolsByDenyRules()        ← 第1层：权限 deny 规则过滤
     ↓
getTools()                      ← 第2层：模式裁剪 + isEnabled 自检
     ↓
assembleToolPool()              ← 合并 MCP → 去重 → 排序
     ↓
isDeferredTool()                ← 判定是否延迟加载
     ↓
filterToolsForAgent()           ← 子 Agent 二次裁剪
```

### 3.2 `buildTool()`：五份契约的工程落地

`buildTool` 是所有 60+ 个内置工具的出厂函数。它的核心职责不是"创建工具"，而是**替开发者填写所有安全属性的默认值**。开发者只提供执行逻辑，系统代填安全宣言。
...
#### 3.7.1 怎么记住？—— `extractDiscoveredToolNames()`
...
#### 3.7.2 断连时怎么通知？—— `getDeferredToolsDelta()`
...
- **日志中的 `carriedFromBoundary`**：`extractDiscoveredToolNames` 统计了从压缩边界恢复的工具数。这行日志的价值在于监控——如果这个数字持续为 0，说明压缩逻辑没有正确保存发现状态。
