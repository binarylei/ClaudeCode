...
`resolveHookPermissionDecision` 和 `hasPermissionsToUseTool` 不是两个独立函数，而是同一条决策链的前后段。前者判断"hook 说了什么"，后者决定"系统规则怎么说"。完整的决策路径可以用一个三分支概括：
...
**入口仲裁：hook 的三路分流。** 决策链的入口是 `resolveHookPermissionDecision`。第一个判断是 `hookPermissionResult?.behavior`——三态 + undefined 实际上只有三条路。
...
这段代码体现了入口仲裁的两个设计选择：**hook allow 绕过的是弹窗，不绕过 deny/ask 规则**——hook 可以替用户点"允许"，但不能替管理员撤销禁令；**hook deny 直接结束**——不需要再查规则。第三条路（无决策或 ask）则交给下一步的完整权限流程。

**完整终局：逐级判定的三层管线。** 规则复核仍然无法决定时，进入 `hasPermissionsToUseTool` 的权限管线——10 个步骤（1a→1b→1c→1d→1e→1f→1g→2a→2b→3）不是教科书式的分类，而是需求迭代的自然沉积。每加一个新需求，就在现有步骤之间"挤"一个新编号。
...
- **两层"规则检查"不是冗余**：入口的 `checkRuleBasedPermissions` 只查 deny/ask 子集（不查 always-allow），因为 hook 已经介入决策，bypass 模式不应自动放行；完整流程的 `hasPermissionsToUseTool` 才是全量规则检查。一个是"有人否决吗？"，一个是"可以放行吗？"，问的是不同的问题。
- **1g 编号在 1e 之后，但代码在 1d 之后**：1g（safetyCheck）紧跟 1d 而不是 1f——因为安全路径检查是从 `tool.checkPermissions` 返回的，不是独立规则。编号反映的是功能加入的时机，不是代码的执行顺序。
- **`behavior: 'passthrough'` 是"沉默的中间立场"**：工具返回 passthrough 意味着"我检查了，没问题，但不替用户做决定"。管线把它转成 `ask` 继续往下走——三态（allow/deny/passthrough）比二态多了一种"沉默"，让工具避免在"允许"和"拒绝"之间被迫选边。
