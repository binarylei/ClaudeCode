...
- **异常时按不安全对待**：`isConcurrencySafe()` 抛异常时返回 `false`——fail-closed。不知道能不能并行，就当不能，代价是性能（多等一轮）。
...
**调度分发**——`query.ts` 主循环中工具执行的集成点，根据是否流式分为：

```
query.ts: queryLoop
  ├─ applyToolResultBudget()              ← per-message 预算
  │
  └─ deps.callModel({ tools })
...
**非流式执行**—— `toolOrchestration.ts`下 `runTools` 的完整调用链：
...
runTools()
  ...
  └─ runToolsSerially()
     └─ for each toolUse:
          ...
          │  ├─ checkRuleBasedPermissions()   ├─ 权限决策：hook 入口 → 三层管线
...
### 3.2 `partitionToolCalls`：连续的归并，不连续的断开
...
- **`acc[last]` 比较保证了"连续"**：`isConcurrencySafe && acc[last]?.isConcurrencySafe` 只和最后一个 batch 比，不和更早的比——如果改成遍历全部去找同类合并，就会破坏因果顺序。

### 3.3 权限决策

权限决策先由 `resolveHookPermissionDecision` 处理 PreToolUse hook 的裁决（外部脚本拦截），再由 `hasPermissionsToUseToolInner` 执行系统内置规则（三层管线）。两段之间有明确的调用关系——hook 裁决的某些分支会进入系统规则。

```
hook 返回决策
  │
  ├─ deny → 直达终点（不用查规则、不用走管线）
  │
  ├─ allow → 先查 deny/ask 规则
  │    ├─ 无规则 → hook allow 生效
  │    ├─ deny 规则 → 规则覆盖 hook
  │    └─ ask 规则 → 降级到'系统内置规则'
  │
...
##### 3.3.1 PreToolUse hook 裁决：resolveHookPermissionDecision

PreToolUse hook 是用户在 `settings.json` 中配置的脚本拦截点，在工具执行前触发，可以返回 allow / deny / ask 三种裁决。`resolveHookPermissionDecision` 根据 `hookPermissionResult?.behavior` 做路由：allow 走 hook 放行路径，deny 走直接拒绝，其余情况（ask 或 hook 未返回决策）统一交给 3.3.2 的系统规则。
...
这段代码体现了入口仲裁的两个设计选择：**hook allow 跳过交互弹窗（替代用户点"确认"），但不跳过 deny/ask 规则（不能替代用户预设的禁令）**；**hook deny 直接结束**——不需要再查规则。

##### 3.3.2 系统规则管线：hasPermissionsToUseToolInner

当 hook 裁决无法直接拒绝时——无论 hook allow + requiresInteraction、hook allow + ask 规则，还是 hook 无决策/ask——都会通过 `canUseTool` 进入系统规则。完整的规则引擎是 `hasPermissionsToUseToolInner`，三层权限管线的全部逻辑都在其中：
- 第 1 层（规则强制执行）：步骤 1a-1g
- 第 2 层（自动放行）：步骤 2a-2b
- 第 3 层（交互决策）：将 `passthrough` 转 `ask` 后启动，具体走 AI 分类器、弹窗还是 auto-deny，由调用方根据运行模式决定。
...
async function hasPermissionsToUseToolInner(
  ...
  // 1f. 内容相关的 ask 规则覆盖 bypassPermissions
  // 1g. 安全路径检查不受 bypass 影响

  // 2a. bypass 模式 → 自动放行
  if (shouldBypassPermissions) return { behavior: 'allow', ... }
  // 2b. 工具在 always-allow 规则中
  const alwaysAllowedRule = toolAlwaysAllowedRule(...)
  if (alwaysAllowedRule) return { behavior: 'allow', ... }

  // 3. passthrough 转 ask，进入交互决策
...
- **`checkRuleBasedPermissions` 问"有人反对吗"，`hasPermissionsToUseToolInner` 问"可以放行吗"**。前者只查 deny/ask 规则，返回 `null` 表示无异议，供 hook allow 路径快速放行；后者是全量检查，必须有终态。一个函数做否决判断，一个做批准判断——职责分离。
- **`passthrough` 是工具的"弃权票"**：工具返回 passthrough 意为"我检查了，没问题，但不替你做决定"，管线在第 3 步转成 `ask` 继续走。三态（allow/deny/passthrough）比二态多了一种"沉默"，让工具不用在"允许"和"拒绝"之间被迫选边。

---

### 3.4 `maybePersistLargeToolResult`：单个结果超限
...
### 3.5 `enforceToolResultBudget`：一轮所有结果超限--贪心选择
