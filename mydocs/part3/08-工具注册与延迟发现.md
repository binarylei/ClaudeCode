...
Claude Code 内置 60+ 个工具，按功能分为以下几类。带 `*` 标记的为条件启用（feature flag 或环境变量控制），并非每个会话都可用。

#### A.1 文件操作类

| 工具名 | 作用 | 条件 |
|--------|------|------|
| `Read` | 读文件（支持文本、图片、PDF） | — |
| `Write` | 覆写/新建文件 | — |
| `Edit` | 精确字符串替换编辑 | — |
| `Glob` | 按 glob 模式匹配文件路径 | 无嵌入式搜索工具时 |
| `Grep` | 按正则搜索文件内容 | 无嵌入式搜索工具时 |
| `NotebookEdit` | 编辑 Jupyter notebook (.ipynb) 单元格 | — |
| `LSP` | 语言服务器协议（跳转定义、查找引用等） | `ENABLE_LSP_TOOL` |
...
|--------|------|------|
| `Bash` | 执行 shell 命令 | — |
| `PowerShell` | 执行 PowerShell 命令 | `isPowerShellToolEnabled` |

#### A.3 网络/搜索类

| 工具名 | 作用 | 条件 |
|--------|------|------|
| `WebSearch` | 网页搜索（默认 US-only） | — |
| `WebFetch` | 抓取 URL 内容转为 markdown | — |
| `WebBrowser` | 完整浏览器渲染 | `WEB_BROWSER_TOOL` |

#### A.4 Agent 编排类

| 工具名 | 作用 | 条件 |
|--------|------|------|
| `Agent` | 派生子 Agent（Fork/Worktree 隔离模式） | — |
| `TaskCreate` | 创建后台任务 | `isTodoV2Enabled` |
| `TaskGet` | 查询任务状态 | `isTodoV2Enabled` |
| `TaskUpdate` | 更新任务 | `isTodoV2Enabled` |
| `TaskList` | 列出所有任务 | `isTodoV2Enabled` |
| `TaskOutput` | 获取任务输出 | — |
| `TaskStop` | 停止任务 | — |
| `SendMessage` | Agent 间发送消息 | — |
| `TeamCreate` | 创建 Agent 团队（Swarm 模式） | `isAgentSwarmsEnabled` |
| `TeamDelete` | 删除 Agent 团队 | `isAgentSwarmsEnabled` |
| `SyntheticOutput` | 以结构化 JSON 返回最终响应（命令模式） | — |

#### A.5 工具发现类

| 工具名 | 作用 | 条件 |
|--------|------|------|
| `ToolSearch` | 搜索可用的延迟工具（含 MCP） | 乐观启用 |
| `ListMcpResources` | 列出 MCP 服务端的资源 | — |
| `ReadMcpResource` | 读取 MCP 服务端的特定资源 | — |

#### A.6 Plan / Workflow 类

| 工具名 | 作用 | 条件 |
|--------|------|------|
| `EnterPlanMode` | 进入计划模式（只读规划，先想后做） | — |
| `ExitPlanMode` | 退出计划模式 | — |
| `Workflow` | 执行多 Agent 编排脚本（ultracode） | `WORKFLOW_SCRIPTS` |
| `VerifyPlanExecution` | 验证计划执行结果 | `CLAUDE_CODE_VERIFY_PLAN` |

#### A.7 隔离执行类

| 工具名 | 作用 | 条件 |
|--------|------|------|
| `EnterWorktree` | 创建并进入隔离的 git worktree | `isWorktreeModeEnabled` |
| `ExitWorktree` | 退出 worktree（保留或删除） | `isWorktreeModeEnabled` |

#### A.8 交互/配置类

| 工具名 | 作用 | 条件 |
|--------|------|------|
| `AskUserQuestion` | 弹窗提问用户 | — |
| `TodoWrite` | 更新任务列表（向用户展示进度） | — |
| `Skill` | 执行已注册的技能 | — |
| `Config` | 获取或设置配置项 | 蚂蚁内部 |
| `SendUserMessage` | 向用户发送消息（主通信通道） | — |

#### A.9 任务调度类

| 工具名 | 作用 | 条件 |
|--------|------|------|
| `CronCreate` | 创建定时任务 | `AGENT_TRIGGERS` |
| `CronDelete` | 删除定时任务 | `AGENT_TRIGGERS` |
| `CronList` | 列出定时任务 | `AGENT_TRIGGERS` |
| `Sleep` | 等待指定时长 | `KAIROS` 或 `PROACTIVE` |
| `RemoteTrigger` | 管理远程定时 Agent（通过 claude.ai API） | `AGENT_TRIGGERS_REMOTE` |
