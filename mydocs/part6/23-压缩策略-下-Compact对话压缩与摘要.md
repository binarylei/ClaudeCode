---
title: "23. 压缩策略（下）：Compact 对话压缩与摘要"
description: "阈值触发分级响应、Session Memory 复用零成本路径、Forked Agent 缓存共享——对话级压缩如何用最少 API 调用将数万 token 的对话历史压缩为结构化摘要"
outline: [2, 3]
---

# 23. 压缩策略（下）：Compact 对话压缩与摘要

[第 22 章](../part6/22-压缩策略-上-Micro-Compact与工具结果清理) 中我们分析了 Micro-Compact——它在每轮 API 调用前自动清理过时工具结果，零额外成本、对用户完全透明。但这里有一个它无法处理的场景：**对话轮次足够多时，即便工具结果全部清空，Assistant 的思考链和 User 的消息本身也足以撑满窗口。** 清理东西只能延缓，不能根治——当"东西本身"就是问题时，需要的不是清理，而是**重写**。
...
## 附录 A：Compact 摘要提示词参考

Auto compact 通过一次 LLM 调用生成摘要，有两条执行路径：**Forked Agent（缓存共享）** 和**独立 Streaming（兜底）**。两条路径使用的 user message 提示词完全相同，差异仅在 system prompt 和工具集。

**A.1 提示词结构**

user message 由三段拼接而成：

```
NO_TOOLS_PREAMBLE + BASE_COMPACT_PROMPT + NO_TOOLS_TRAILER
```

如果有自定义 compact 指令（通过 hook 或用户配置注入），追加在 `BASE_COMPACT_PROMPT` 之后、`NO_TOOLS_TRAILER` 之前。

**A.2 NO_TOOLS_PREAMBLE**
...
- **System Prompt**：复用主线程的完整 system prompt
- **工具集**：继承主线程全部工具，通过 `canUseTool` 全部拦截 + `NO_TOOLS_PREAMBLE` 双重禁止
- **maxTurns**：1
- **Thinking Config**：继承主线程配置，不设 `maxOutputTokens`（设了会改变 thinking config 破坏缓存 key）
...
- **System Prompt**：
  ...
  ```
- **工具集**：仅 `FileReadTool`
- **Thinking Config**：`{ type: 'disabled' }`
- **maxOutputTokens**：取 `COMPACT_MAX_OUTPUT_TOKENS` 和模型最大输出的较小值

**A.6 输出后处理**

LLM 返回的原始摘要经过两步处理：

1. **剥离 `<analysis>` 块**——这是思考草稿，摘要完成后无保留价值
2. **替换 `<summary>` 标签**——将 `<summary>...</summary>` 替换为 `Summary:\n...`
