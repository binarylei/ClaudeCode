---
title: "06. 核心循环（下）：Agent Loop 迭代机制"
description: "工具调用循环的 Harness 实现、伪代码解析与状态机"
outline: [2, 3]
---

# 06. 核心循环（下）：Agent Loop 迭代机制

本章与[第 5 章](../part2/05-核心循环-上-QueryEngine会话层)是上下两篇：上一章覆盖外层（会话管理、结果判定），本章聚焦内层——`queryLoop()` 的 ReAct 循环：调 API 前的预处理流水线、拿到响应后的 Continue/Terminal 状态分支、以及让错误可自愈的恢复路径。全文围绕 [src/query.ts](../../src/query.ts) 约 900 行源码展开。
...
在[第 5 章](../part2/05-核心循环-上-QueryEngine会话层)中，我们建立了 QueryEngine 的分层结构——外层 `QueryEngine.submitMessage()` 管会话生命周期，内层 `queryLoop()` 管 Agent Loop 迭代。这里从内层的视角重新审视：
...
> **前置阅读**：本章的源码分析建立在[第 5 章](../part2/05-核心循环-上-QueryEngine会话层)的架构全景之上。阅读本章前，请先了解外层与内层的分工（外层 `QueryEngine` + 内层 `queryLoop`）。
...
另外，`QueryEngine.submitMessage()` 本身也可能在调用 `query()` 之前提前返回 Terminal，详见[第 5 章 3.6 节](../part2/05-核心循环-上-QueryEngine会话层#_3-6-外层-终止路径全景)。
...
- 架构全景和调用链路见[第 5 章](../part2/05-核心循环-上-QueryEngine会话层)。
